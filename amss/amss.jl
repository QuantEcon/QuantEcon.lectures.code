...
"""
mutable struct BellmanEquation{TP <: Para, TI <: Integer, TR <: Real}
    para::TP
    S::TI
    xbar::Array{TR}
    time_0::Bool
...
"""
function solve_time1_bellman{TR <: Real}(para::Para{TR}, mugrid::AbstractArray)
    Pi = para.Pi
    S = size(para.Pi, 1)
    ...
    # cf = Array{LinInterp}(S, S)
    # nf = Array{LinInterp}(S, S)
...
Tau(pab::Planners_Allocation_Bellman, c::Real, n::Real) =
    Tau(pab, [c], [n])
...
"""
function simulate{TF <: AbstractFloat}(pab::Planners_Allocation_Bellman,
                   B_::TF, s_0::Integer, T::Integer,
                   sHist::Vector=QuantEcon.simulate(pab.mc, T, init=s_0))
...
"""
function BellmanEquation{TF <: AbstractFloat}(para::Para{TF},
...
function get_policies_time1(T::BellmanEquation,
    ...
    if para.transfers == true
        ...
        if para.n_less_than_one == true
            ub = vcat(100*ones(S_possible)-G[sprimei_possible],
                      ones(S_possible)*xbar[2], 100*ones(S_possible))
        else
